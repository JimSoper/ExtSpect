Ext.define(
	'uxExtSpect.view.tree.datalist.TreeList',
	{  extend: 'uxExtSpect.view.ExtSpectDataList',
		...
		selectValue: function ( value ) {
			...
			if ( store )
			{ // a list that is not visible mght not have a store
		...
		// Above for Safari on Mac. Below for Safari on iPad
		treeSpaces: '&emsp;', // to the left of the bars
		...
		computeVerticalBars: function ( object, objectString ) {
			...
			for ( var len = totalCountsLen - 1; index < len; index ++ ) {
				if ( counts[ index ] < totalCounts[ index ] )
				{ chars.push( this.verticalBarChar ); } // ┃
