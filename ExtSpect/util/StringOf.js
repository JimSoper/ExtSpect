...
if ( ! uxExtSpect.util.StringOf )
{ uxExtSpect.util.StringOf = {}; }
...
uxExtSpect.util.StringOf.to$ = function ( value ) {
	...
	{    // '123' instanceof String -> false
		case 'string' :       // : return 	this.ellipsis$( value )
			return this.leftDoubleQuoteChar + this.ellipsis$( value ) + this.rightDoubleQuoteChar;
		case 'number' :
		...
		// These do not seem to be occuring. See  ! ( objct instanceof Object ) test
		// in objectData$
	}

	// If there is an extspectString() method, use it
	var fnctn = value.extspectString;
	if ( fnctn instanceof Function )
	{ return fnctn.call( value );}
	// else
...
uxExtSpect.util.StringOf.object$ =
	function toString$object$ ( objct, type$ ) {
		var data$ = this.objectData$( objct );
		type$ = this.objectType$( objct, type$ );
		return strng = ( objct instanceof Element ) ?
		               this.element$( type$, data$ ) :
		               this.embrace$( type$, data$ );
	};

// ---------- uxExtSpect.util.StringOf.objectType$

uxExtSpect.util.StringOf.objectType$ =
	function toString$objectType$ ( object, typeString ) {

		if ( ! ( object instanceof Object ) ) {
			console.warn( 'toString$objectType$', '( objct NOT instanceof Object )',
			              'type$=', typeString, object );
		}

		typeString = object.type || object.localName || object.nodeName || typeString;
...
uxExtSpect.util.StringOf.objectData$ =
	function StringOf$objectData$ ( object ) {

		var dataString = object._itemId ||  object.id || object.name || object.className || object.xtype ||
			object.value || object.nodeValue;

		if ( ! dataString && ( 'nodeName' in object ) ) {
			dataString = object.src || object.href;
			if ( dataString )
			{ dataString = this.lastPartOfName( dataString, 2, '/' ) }
		}
		// Ext.getClassName(), Ext.ClassManager.getDisplayName( Mixed object )
		if ( object.hasOwnProperty( "$className" ) ) { // Sencha
			dataString = this.lastPartOfName( object.$className || dataString, 2, '.' );
		}

		if ( ( "xtype" in object ) && ( object instanceof Ext.Base ) ) {
			var text = object._text || object._iconCls; // Ext.Button
			if ( text ) {
		...
		if ( dataString )
		{ dataString = this.quote$( dataString ); }
		else { dataString = this.objectData$Bis( dataString ); }

		return dataString;
	};
uxExtSpect.util.StringOf.objectData$._name = 'uxExtSpect.util.StringOf.objectData$';
...
//	}
//	else {
//		dataString = object.id || object.name || '';
//
//		if ( object.hasOwnProperty( "$className" ) ) // Sencha
//		{
//			dataString = this.lastPartOfName( object.$className || dataString, 2 );
...
//				{
//					typeString = object.localName || object.nodeName || typeString
...
//
//	if ( ! dataString )
//	{ dataString = this.objectData$( object ); }
...
uxExtSpect.util.StringOf.bracket$ = function ( type$_, data$ ) {
	var strng = '[' + data$ + ']';
	if ( type$_ )
	{ strng = type$_ + strng; }
	return strng;
};
uxExtSpect.util.StringOf.bracket$._name = 'uxExtSpect.util.StringOf.bracket$';
...
uxExtSpect.util.StringOf.objectData$Bis = function ( object ) {
	var propertyCount = 0;
	var methodCount = 0;
	var dataString = '';
	this.to$_depth ++;
	...
	{ dataString = '0...'; }
		...
		{ dataString = this.objectProperties$( object ); }
		else {
...
;
uxExtSpect.util.StringOf.objectData$Bis._name = 'uxExtSpect.util.StringOf.objectData$Bis';
...
uxExtSpect.util.StringOf.functionName = function ( fnctn ) {
	...
	// $name is in Sencha Touch
	var name = fnctn.name || fnctn._name || fnctn.$name || fnctn.displayName || 'anonFn';
...
uxExtSpect.util.StringOf.constructorName._name = 'uxExtSpect.util.StringOf.constructorName';
