Ext.define(
	...
	{  extend: 'uxExtSpect.view.ExtSpectNavigationView',
		...
		requires: [
			'uxExtSpect.view.object.tabpanel.ArrayTabPanel',
		...
		// or use autoDestroy: true (the default)

		// pop : function ( ) {
		// var currentTabPanel = this._activeItem
		//	this.callParent( arguments )
		// currentTabPanel.destroy()
		//  it is sufficient to make sure the parent method gets called
		...
		pushNewPropertiesPanel: function ( newObject ) {
			if ( this.canDrillDown( newObject ) ) {
				...
				var newTabPanel = Ext.create( tabPanelString, { title: title } );

				newTabPanel.setRootObject( newObject );
				this.push( newTabPanel );
		...
		pushNewArrayPanel: function ( array, previousProperty, previousTabPanel, panelClass ) {
			...
			if ( (array.length === 1) && (array[0] instanceof Object) ) {
				...
				var newTabPanel =
					Ext.create( panelClass || 'uxExtSpect.view.object.tabpanel.ArrayTabPanel',
					            { title: title } );
